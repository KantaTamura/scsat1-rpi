...
int main()
{
	...
	if (error != CSP_ERR_NONE) {
		csp_print("failed to add KISS interface [/dev/ttyS0], error: %d\n", error);
		exit(1);
	}
	...
	if (CSP_USE_RTABLE) {
		csp_rtable_set(RPI_PICO_UART_ADDR, csp_id_get_host_bits(), usart_iface,
			       CSP_NO_VIA_ADDRESS);
		csp_rtable_set(MAIN_OBC_CAN_ADDR, csp_id_get_host_bits(), can_iface,
			       CSP_NO_VIA_ADDRESS);
	}
